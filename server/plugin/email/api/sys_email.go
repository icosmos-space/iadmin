...
import (
	...
	email_response "github.com/icosmos-space/iadmin/server/plugin/email/model/response"
	"github.com/icosmos-space/iadmin/server/plugin/email/service"
	"go.uber.org/zap"
)
...
func (s *EmailApi) EmailTest(c *gin.Context) {
	...
	if err != nil {
		global.IADMIN_LOG.Error("发送失败!", zap.Error(err))
