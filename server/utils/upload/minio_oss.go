...
func (m *Minio) UploadFile(file *multipart.FileHeader) (filePathres, key string, uploadErr error) {
	...
	if err != nil {
		global.IADMIN_LOG.Error("读取文件失败", zap.Any("err", err.Error()))
	...
	if global.IADMIN_CONFIG.Minio.BasePath == "" {
		filePathres = "uploads" + "/" + time.Now().Format("2006-01-02") + "/" + filename
	} else {
		filePathres = global.IADMIN_CONFIG.Minio.BasePath + "/" + time.Now().Format("2006-01-02") + "/" + filename
	}
	...
	info, err := m.Client.PutObject(ctx, global.IADMIN_CONFIG.Minio.BucketName, filePathres, &filecontent, file.Size, minio.PutObjectOptions{ContentType: contentType})
	if err != nil {
		global.IADMIN_LOG.Error("上传文件到minio失败", zap.Any("err", err.Error()))
		return "", "", errors.New("上传文件到minio失败, err:" + err.Error())
	}
	return global.IADMIN_CONFIG.Minio.BucketUrl + "/" + info.Key, filePathres, nil
}
