...
func (baseMenuService *BaseMenuService) DeleteBaseMenu(id int) (err error) {
	err = global.IADMIN_DB.First(&system.SysBaseMenu{}, "parent_id = ?", id).Error
	if err == nil {
		return errors.New("此菜单存在子菜单不可删除")
	}
	var menu system.SysBaseMenu
	err = global.IADMIN_DB.First(&menu, id).Error
	if err != nil {
		return errors.New("记录不存在")
	}
	err = global.IADMIN_DB.First(&system.SysAuthority{}, "default_router = ?", menu.Name).Error
	if err == nil {
	...
	err = global.IADMIN_DB.Transaction(func(tx *gorm.DB) error {
		...
		if oldMenu.Name != menu.Name {
			if !errors.Is(tx.Where("id <> ? AND name = ?", menu.ID, menu.Name).First(&system.SysBaseMenu{}).Error, gorm.ErrRecordNotFound) {
				global.IADMIN_LOG.Debug("存在相同name修改失败")
...
func (baseMenuService *BaseMenuService) GetBaseMenuById(id int) (menu system.SysBaseMenu, err error) {
	err = global.IADMIN_DB.Preload("MenuBtn").Preload("Parameters").Where("id = ?", id).First(&menu).Error
	return
}
