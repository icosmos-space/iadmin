...
func (a *autoCodeSqlite) GetDB(businessDB string) (data []response.Db, err error) {
	...
	if businessDB == "" {
		err = global.IADMIN_DB.Raw(sql).Find(&databaseList).Error
	} else {
		err = global.IADMIN_DBList[businessDB].Raw(sql).Find(&databaseList).Error
	}
...
func (a *autoCodeSqlite) GetTables(businessDB string, dbName string) (data []response.Table, err error) {
	...
	tabelNames := []string{}
	if businessDB == "" {
		err = global.IADMIN_DB.Raw(sql).Find(&tabelNames).Error
	} else {
		err = global.IADMIN_DBList[businessDB].Raw(sql).Find(&tabelNames).Error
	}
...
func (a *autoCodeSqlite) GetColumn(businessDB string, tableName string, dbName string) (data []response.Column, err error) {
	...
	if businessDB == "" {
		err = global.IADMIN_DB.Raw(sql).Scan(&columnInfos).Error
	} else {
		err = global.IADMIN_DBList[businessDB].Raw(sql).Scan(&columnInfos).Error
	}
