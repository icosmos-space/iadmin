...
func (s *autoCodeMysql) GetDB(businessDB string) (data []response.Db, err error) {
	...
	if businessDB == "" {
		err = global.IADMIN_DB.Raw(sql).Scan(&entities).Error
	} else {
		err = global.IADMIN_DBList[businessDB].Raw(sql).Scan(&entities).Error
	}
	return entities, err
}
...
func (s *autoCodeMysql) GetTables(businessDB string, dbName string) (data []response.Table, err error) {
	...
	if businessDB == "" {
		err = global.IADMIN_DB.Raw(sql, dbName).Scan(&entities).Error
	} else {
		err = global.IADMIN_DBList[businessDB].Raw(sql, dbName).Scan(&entities).Error
	}
...
ORDER BY 
    c.ORDINAL_POSITION;`
	if businessDB == "" {
		err = global.IADMIN_DB.Raw(sql, tableName, dbName).Scan(&entities).Error
	} else {
		err = global.IADMIN_DBList[businessDB].Raw(sql, tableName, dbName).Scan(&entities).Error
	}
