...
func (userService *UserService) Register(u system.SysUser) (userInter system.SysUser, err error) {
	...
	if !errors.Is(global.IADMIN_DB.Where("username = ?", u.Username).First(&user).Error, gorm.ErrRecordNotFound) { // 判断用户名是否注册
		return userInter, errors.New("用户名已注册")
	}
	// 否则 附加uuid 密码hash加密 注册
	u.Password = utils.BcryptHash(u.Password)
	u.UUID = uuid.New()
	err = global.IADMIN_DB.Create(&u).Error
	return u, err
}
...
func (userService *UserService) Login(u *system.SysUser) (userInter *system.SysUser, err error) {
	if nil == global.IADMIN_DB {
		return nil, fmt.Errorf("db not init")
	}

	var user system.SysUser
	err = global.IADMIN_DB.Where("username = ?", u.Username).Preload("Authorities").Preload("Authority").First(&user).Error
	if err == nil {
...
func (userService *UserService) ChangePassword(u *system.SysUser, newPassword string) (err error) {
	var user system.SysUser
	err = global.IADMIN_DB.Select("id, password").Where("id = ?", u.ID).First(&user).Error
	if err != nil {
		return err
	}
	if ok := utils.BcryptCheck(u.Password, user.Password); !ok {
	...
	err = global.IADMIN_DB.Model(&user).Update("password", pwd).Error
	return err
}
...
func (userService *UserService) GetUserInfoList(info systemReq.GetUserList) (list interface{}, total int64, err error) {
	...
	db := global.IADMIN_DB.Model(&system.SysUser{})
...
func (userService *UserService) SetUserAuthority(id uint, authorityId uint) (err error) {
	...
	assignErr := global.IADMIN_DB.Where("sys_user_id = ? AND sys_authority_authority_id = ?", id, authorityId).First(&system.SysUserAuthority{}).Error
	...
	err = global.IADMIN_DB.Where("sys_authority_authority_id = ?", authorityId).Find(&authorityMenu).Error
	if err != nil {
		return err
	}
	...
	err = global.IADMIN_DB.Preload("Parameters").Where("id in (?)", authorityMenuIDs).Find(&authorityMenus).Error
	if err != nil {
		return err
	}
	hasMenu := false
	...
	err = global.IADMIN_DB.Model(&system.SysUser{}).Where("id = ?", id).Update("authority_id", authorityId).Error
	return err
}
...
func (userService *UserService) SetUserAuthorities(adminAuthorityID, id uint, authorityIds []uint) (err error) {
	return global.IADMIN_DB.Transaction(func(tx *gorm.DB) error {
		...
		if TxErr != nil {
			global.IADMIN_LOG.Debug(TxErr.Error())
...
func (userService *UserService) SetUserInfo(req system.SysUser) error {
	return global.IADMIN_DB.Model(&system.SysUser{}).
...
func (userService *UserService) SetSelfSetting(req common.JSONMap, uid uint) error {
	return global.IADMIN_DB.Model(&system.SysUser{}).Where("id = ?", uid).Update("origin_setting", req).Error
}
...
func (userService *UserService) GetUserInfo(uuid uuid.UUID) (user system.SysUser, err error) {
	...
	err = global.IADMIN_DB.Preload("Authorities").Preload("Authority").First(&reqUser, "uuid = ?", uuid).Error
	if err != nil {
		return reqUser, err
	}
...
func (userService *UserService) FindUserByUuid(uuid string) (user *system.SysUser, err error) {
	var u system.SysUser
	if err = global.IADMIN_DB.Where("uuid = ?", uuid).First(&u).Error; err != nil {
...
func (userService *UserService) ResetPassword(ID uint, password string) (err error) {
	err = global.IADMIN_DB.Model(&system.SysUser{}).Where("id = ?", ID).Update("password", utils.BcryptHash(password)).Error
	return err
}
