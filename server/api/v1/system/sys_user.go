...
func (b *BaseApi) Login(c *gin.Context) {
	...
	key := c.ClientIP()
	// 判断验证码是否开启
	openCaptcha := global.IADMIN_CONFIG.Captcha.OpenCaptcha               // 是否开启防爆次数
	openCaptchaTimeOut := global.IADMIN_CONFIG.Captcha.OpenCaptchaTimeOut // 缓存超时时间
	...
	if err != nil {
		global.IADMIN_LOG.Error("登陆失败! 用户名不存在或者密码错误!", zap.Error(err))
		// 验证码次数+1
		global.BlackCache.Increment(key, 1)
	...
	if user.Enable != 1 {
		global.IADMIN_LOG.Error("登陆失败! 用户被禁止登录!")
		// 验证码次数+1
		global.BlackCache.Increment(key, 1)
...
func (b *BaseApi) TokenNext(c *gin.Context, user system.SysUser) {
	...
	if err != nil {
		global.IADMIN_LOG.Error("获取token失败!", zap.Error(err))
	...
	if jwtStr, err := jwtService.GetRedisJWT(user.Username); err == redis.Nil {
		if err := utils.SetRedisJWT(token, user.Username); err != nil {
			global.IADMIN_LOG.Error("设置登录状态失败!", zap.Error(err))
...
func (b *BaseApi) Register(c *gin.Context) {
	...
	if err != nil {
		global.IADMIN_LOG.Error("注册失败!", zap.Error(err))
...
func (b *BaseApi) ResetPassword(c *gin.Context) {
	...
	if err != nil {
		global.IADMIN_LOG.Error("重置失败!", zap.Error(err))
