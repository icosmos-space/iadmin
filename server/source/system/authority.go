...
func (i *initAuthority) InitializeData(ctx context.Context) (context.Context, error) {
	...
	entities := []sysModel.SysAuthority{
		{AuthorityId: constant.RoleSuperAdminID, AuthorityName: constant.RoleSuperAdminName, ParentId: utils.Pointer[uint](0), DefaultRouter: "dashboard"},
		{AuthorityId: constant.RoleSystemAdminID, AuthorityName: constant.RoleSystemAdminName, ParentId: utils.Pointer[uint](0), DefaultRouter: "dashboard"},
		{AuthorityId: constant.RoleNormalUserID, AuthorityName: constant.RoleNormalUserName, ParentId: utils.Pointer[uint](0), DefaultRouter: "dashboard"},
	}
	...
	if err := db.Model(&entities[0]).Association("DataAuthorityId").Replace(
		[]*sysModel.SysAuthority{
			...
			{AuthorityId: constant.RoleNormalUserID},
		}); err != nil {
...
func (i *initAuthority) DataInserted(ctx context.Context) bool {
	...
	if errors.Is(db.Where("authority_id = ?", constant.RoleNormalUserID).
