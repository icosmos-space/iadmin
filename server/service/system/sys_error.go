...
func (sysErrorService *SysErrorService) CreateSysError(ctx context.Context, sysError *system.SysError) (err error) {
	...
	err = global.IADMIN_DB.Create(sysError).Error
	return err
}
...
func (sysErrorService *SysErrorService) DeleteSysError(ctx context.Context, ID string) (err error) {
	err = global.IADMIN_DB.Delete(&system.SysError{}, "id = ?", ID).Error
	return err
}
...
func (sysErrorService *SysErrorService) DeleteSysErrorByIds(ctx context.Context, IDs []string) (err error) {
	err = global.IADMIN_DB.Delete(&[]system.SysError{}, "id in ?", IDs).Error
	return err
}
...
func (sysErrorService *SysErrorService) UpdateSysError(ctx context.Context, sysError system.SysError) (err error) {
	err = global.IADMIN_DB.Model(&system.SysError{}).Where("id = ?", sysError.ID).Updates(&sysError).Error
	return err
}
...
func (sysErrorService *SysErrorService) GetSysError(ctx context.Context, ID string) (sysError system.SysError, err error) {
	err = global.IADMIN_DB.Where("id = ?", ID).First(&sysError).Error
	return
}
...
func (sysErrorService *SysErrorService) GetSysErrorInfoList(ctx context.Context, info systemReq.SysErrorSearch) (list []system.SysError, total int64, err error) {
	...
	// 创建db
	db := global.IADMIN_DB.Model(&system.SysError{}).Order("created_at desc")
...
func (sysErrorService *SysErrorService) GetSysErrorSolution(ctx context.Context, ID string) (err error) {
	// 立即更新为处理中
	err = global.IADMIN_DB.WithContext(ctx).Model(&system.SysError{}).Where("id = ?", ID).Update("status", "处理中").Error
	if err != nil {
		return err
	}
	...
	go func(id string) {
		...
		_ = global.IADMIN_DB.Model(&system.SysError{}).Where("id = ?", id).First(&se).Error
		...
		if data, err := (&AutoCodeService{}).LLMAuto(context.Background(), llmReq); err == nil {
			...
			_ = global.IADMIN_DB.Model(&system.SysError{}).Where("id = ?", id).Updates(map[string]interface{}{"status": "处理完成", "solution": solution}).Error
		} else {
			// 即使生成失败也标记为完成，避免任务卡住
			_ = global.IADMIN_DB.Model(&system.SysError{}).Where("id = ?", id).Update("status", "处理失败").Error
		}
	}(ID)
